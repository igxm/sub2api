...
func TestAccountHandlerGetAvailableModels_GrokUsesXAIDefaults(t *testing.T) {
	svc := &availableModelsAdminService{
		stubAdminService: newStubAdminService(),
		account: service.Account{
			ID:       46,
	...
	_, hasImage := ids["grok-imagine-image"]
	_, hasImageQuality := ids["grok-imagine-image-quality"]
	require.True(t, hasImage)
	require.True(t, hasImageQuality)
}
