...
Sub2API 支持通过 xAI OAuth 接入 Grok 订阅账号，并将 OpenAI 兼容的文本、图片和视频请求转发到 xAI。

- 平台名称：`grok`
- 账号类型：OAuth 订阅账号
- 文本端点：`/v1/responses` 和 `/responses`
- 图片端点：`/v1/images/generations`、`/v1/images/edits`
- 视频端点：`/v1/videos/generations`、`/v1/videos/edits`、`/v1/videos/extensions`、`/v1/videos/{request_id}`
- 文本模型：`grok-4.3`、`grok-build-0.1`、`grok-4.20-0309-reasoning`、`grok-4.20-0309-non-reasoning`、`grok-4.20-multi-agent-0309`
- 媒体模型：`grok-imagine-image`、`grok-imagine-image-quality`、`grok-imagine-video`
- 兼容 xAI SDK 风格字段：图片 edits 支持 `image_url`、`image_urls`；视频请求支持 `image_url`、`reference_image_urls`、`video_url`。

暂不支持 Grok Chat Completions、TTS、转录、浏览器自动化、Cookie 和 Grok Web 抓取。
